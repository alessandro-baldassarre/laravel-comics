<div>
  <div class="menu-buy">
    <div class="my-container">
          <div class="buy-card">
          <img src="{{asset("assets/images/buy-comics-digital-comics.png")}}" alt="" />
          <h4>digital comics</h4>
          </div>
          <div class="buy-card">
          <img src="{{asset("assets/images/buy-comics-merchandise.png")}}" alt="" />
          <h4>dc merchandise</h4>
          </div>
          <div class="buy-card">
          <img src="{{asset("assets/images/buy-comics-subscriptions.png")}}" alt="" />
          <h4>subscription</h4>
          </div>
          <div class="buy-card">
          <img src="{{asset("assets/images/buy-comics-shop-locator.png")}}" alt="" />
          <h4>comic shop locator</h4>
          </div>
          <div class="buy-card">
          <img src="{{asset("assets/images/buy-dc-power-visa.svg")}}" alt="" />
          <h4>dc power visa</h4>
          </div>
    </div>
  </div>
</div>

<div>
    <div class="footer-menu">
      <div class="my-container">
        <div class="footer-menu-links">
          <div>
            <h5>dc comics</h5>
            <ul>
              <li>Characters</li>
              <li>Comics</li>
              <li>Movies</li>
              <li>TV</li>
              <li>Games</li>
              <li>Videos</li>
              <li>News</li>
            </ul>
            <h5>shop</h5>
            <ul>
                <li>Shop DC</li>
                <li>Shop DC Collectibles</li>
            </ul>
          </div>
          <div>
            <h5>dc</h5>
            <ul>
                <li>Term Of Use</li>
                <li>Privacy policy (new)</li>
                <li>Ad Choises</li>
                <li>Advertising</li>
                <li>Jobs</li>
                <li>Subscription</li>
                <li>Talent Workshops</li>
                <li>CPSC Certificates</li>
                <li>Ratings</li>
                <li>Shop Help</li>
                <li>Contacts Us</li>
            </ul>
          </div>
          <div>
            <h5>sites</h5>
            <ul>
                <li>DC</li>
                <li>MAD Magazine</li>
                <li>DC Kids</li>
                <li>DC Universe</li>
                <li>DC Power Visa</li>
            </ul>
          </div>
        </div>
        <div class="footer-menu-logo">
          <img src="{{asset("assets/images/dc-logo-bg.png")}}" alt="dc logo" />
        </div>
      </div>
    </div>
    <div class="footer-bottom">
      <div class="my-container">
        <div class="footer-button">
          <a href="#">sign-up now!</a>
        </div>
        <div class="footer-social">
          <a href="#">follow us</a>
          <a href="#"
            ><img src="{{asset("assets/images/footer-facebook.png")}}" alt="facebook logo" />
          </a>
          <a href="#"
            ><img src="{{asset("assets/images/footer-twitter.png")}}" alt="twitter logo" />
          </a>
          <a href="#"
            ><img src="{{asset("assets/images/footer-youtube.png")}}" alt="yootube logo" />
          </a>
          <a href="#"
            ><img src="{{asset("assets/images/footer-pinterest.png")}}" alt="pinterest logo"
          /></a>
          <a href="#"
            ><img src="{{asset("assets/images/footer-periscope.png")}}" alt="periscope logo"
          /></a>
        </div>
      </div>
    </div>
  </div>